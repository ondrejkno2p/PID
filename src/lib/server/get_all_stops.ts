...
export let stops:Array<{name:string,id:string,lat:number,lon:number}>=[];
golem_stops.forEach((golemStop:golem_stop)=>{
    ...
    if (name!=null && id!=null && id.endsWith('P')){
        stops=[...stops,{name:name,id:id,lat:golemStop.geometry.coordinates[1],lon:golemStop.geometry.coordinates[0]}];
    }
});
