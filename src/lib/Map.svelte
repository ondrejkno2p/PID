<script lang="ts">
  ...
  $: url =
    url_static_maps + "?" + new URLSearchParams(geoapify_params).toString();
  ...
  const get_bounds = (
    ...
    if (n <= 1) {
      ...
      dlat = 1.2 * dlat;
      let z = Math.log2(
        180 / Math.max(dlon, dlat / Math.cos((Math.PI * center.lat) / 180.0))
      );
  ...
  onNavigate(async (navigation) => {
    if (
      navigation.to?.params?.stop_name &&
      navigation.from?.params?.stop_name &&
      navigation.to?.params?.stop_name != navigation.from?.params?.stop_name
    )
      preloaded_image = preload(url);
  });
...
<div
  class="card h-fit m-0 overflow-hidden !rounded-none xl:!rounded-container-token"
>
