...
const GolemDepartureToPlatDeparture = (golemDeparture:golem_departure)=>{
    return {
        ...
        delay    : ((golemDeparture.delay.seconds==null || golemDeparture.delay.seconds<0) ? 0 : golemDeparture.delay.seconds ) as number,
    } as departure
}
...
export const GET = (async ({url, setHeaders, url: rurl}) => {
    ...
    const mode = url.searchParams.get('mode')?url.searchParams.get('mode') as modes:"departures";
    ...
    if(mode!='all'){
        ...
        const new_url = url_departure_board+"?"+new URLSearchParams(golemParams).toString();
        ...
        if(id!=null){
            ...
            golem_departures.forEach((golemDeparture:golem_departure) => {
                departures=[...departures,GolemDepartureToPlatDeparture(golemDeparture)];
            });
        }
        return json({departures:(mode=="departures"?departures:null), arrivals:(mode=="arrivals"?departures:null)});
    }
    else if (mode=='all'){
        golemParams.append("mode","departures")
        const departuresUrl = url_departure_board+"?"+new URLSearchParams(golemParams).toString();
        golemParams.set("mode","arrivals")
        const arrivalsUrl = url_departure_board+"?"+new URLSearchParams(golemParams).toString();
        const [golemDeparturesResponse,golemArrivalsResponse] = await Promise.all([fetch(departuresUrl,options),fetch(arrivalsUrl,options)]);
        ...
        const [golemDeparturesBody,golemArrivalsBody] = await Promise.all([golemDeparturesResponse.json(),golemArrivalsResponse.json()]);
        const [golemDepartures,golemArrivals]=[golemDeparturesBody.departures,golemArrivalsBody.departures];
        let departures : Array<departure> = [];
