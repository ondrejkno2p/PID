    ...
    try {
        ...
        let arrivals : Array<departure> = [];
            golem_departures.forEach((golem_departure:golem_departure) => {
                ...
                predicted: new Date(golem_departure.arrival_timestamp.predicted as string),
                scheduled: new Date(golem_departure.arrival_timestamp.scheduled as string),
        ...
        return new Response(JSON.stringify(arrivals),{status:200});
    } catch (error) {
