...
export const GET: RequestHandler = (async ({ url, setHeaders }) => {
  ...
  try {
    const [cache, ttl] = await Promise.all([
      redis_client.get("stops/" + stop_name),
      redis_client.ttl("stops/" + stop_name),
    ]);
    if (cache) {
    ...
    golem_stops.forEach((golem_stop: golem_stop) => {
      if (
        golem_stop.properties.location_type == 0 &&
        golem_stop.properties.stop_id.endsWith("P")
      ) {
        stops = [
          ...stops,
          {
